...
model = dict(
    ...
    rpn_head=dict(
        ...
        feat_channels=256,
        anchor_scales=[4,8],
        anchor_ratios=[0.3,0.5, 1.0, 2.0,3.0],
...
test_cfg = dict(
    ...
    rcnn=dict(
        ...
        score_thr = 0.0001, nms = dict(type='py_cpu_nms_poly_fast', iou_thr=0.1), max_per_img = 2000)
...
data_root = '/media/ubuntu/data/huojianjun/科目四初赛第一阶段/'
img_norm_cfg = dict(
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'train_all_all/train_all_all.json',
        img_prefix=data_root + 'train_all_all/images/',
        img_scale=[(1222,1222)],
        ...
        # extra_aug=dict(photo_metric_distortion=dict(),random_crop=dict(),),
        ),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'test/testA.json',
...
log_level = 'INFO'
work_dir = './work_dirs/faster_rcnn_RoITrans_r101_all_aug_rote_1333_crop_rote'
load_from = None
resume_from = './work_dirs/faster_rcnn_RoITrans_r101_all_aug_rote_1333_crop_rote/epoch_280.pth'
workflow = [('train', 1)]
