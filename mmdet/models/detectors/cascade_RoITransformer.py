...
class Cascade_RoITransformer(BaseDetector, RPNTestMixin):
    ...
    def __init__(self,
        ...
        super(Cascade_RoITransformer, self).__init__()
        ...
        if rbbox_head is not None:
            ...
            if not isinstance(rbbox_roi_extractor, list):
                rbbox_roi_extractor = [
                    rbbox_roi_extractor for _ in range(num_stages)
                ]
            if not isinstance(rbbox_head, list):
                rbbox_head = [rbbox_head for _ in range(num_stages)]
            assert len(rbbox_roi_extractor) == len(rbbox_head) == self.num_stages
            for roi_extractor, head in zip(rbbox_roi_extractor, rbbox_head):
                self.rbbox_roi_extractor.append(
                    builder.build_roi_extractor(roi_extractor))
                self.rbbox_head.append(builder.build_head(head))
        ...
        for i in range(self.num_stages):
            ...
            if self.with_rbbox:
                ...
                for j in range(num_imgs):
                    assign_result = bbox_assigner.assign(
                        rotated_proposal_list[j], gt_bboxes[j], gt_bboxes_ignore[j],
                        gt_labels[j])
                    sampling_result = bbox_sampler.sample(
                        assign_result,
                        proposal_list[j],
