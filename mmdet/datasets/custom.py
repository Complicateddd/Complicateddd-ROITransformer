...
class CustomDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.img_transform2 = ImageTransform(
            size_divisor=self.size_divisor, **self.img_norm_cfg)
    ...
    def prepare_train_img(self, idx):
        ...
        # img_origin=mmcv.imread(osp.join(self.img_prefix, img_info['filename']))
        ...
        #                                                gt_labels)
        if self.extra_aug is not None:
            img, gt_bboxes, gt_masks,gt_labels = self.extra_aug(img, gt_bboxes,gt_masks,
                                                       gt_labels)
        ...
        # print(self.img_scales, self.multiscale_mode)
        ...
        # _, _, pad_shape_2, scale_factor_2 = self.img_transform2(
        #     img_origin, img_scale, flip, keep_ratio=self.resize_keep_ratio)
        ...
        if self.with_mask:
            # if self.extra_aug:
            #     gt_masks = self.mask_transform(gt_masks, pad_shape_2,
            #                                scale_factor_2, flip)
            # else:
    ...
    def prepare_test_img(self, idx):
        ...
        def prepare_single(img, scale, flip,img_info ,proposal=None):
        ...
        def prepare_rotation_single(img, scale, flip, angle,img_info):
        ...
        for scale in self.img_scales:
            _img, _img_meta, _proposal = prepare_single(
                img, scale, False,img_info, proposal)
            imgs.append(_img)
            img_metas.append(DC(_img_meta, cpu_only=True))
            proposals.append(_proposal)
            if self.flip_ratio > 0:
                _img, _img_meta, _proposal = prepare_single(
                    img, scale, True,img_info, proposal)
                imgs.append(_img)
                img_metas.append(DC(_img_meta, cpu_only=True))
                proposals.append(_proposal)
        if self.rotate_test_aug is not None :
            ...
            for angle in [90, 180, 270]:
                ...
                for scale in self.img_scales:
                    _img, _img_meta,  = prepare_rotation_single(
                        img, scale, False, angle,img_info)
                    imgs.append(_img)
                    img_metas.append(DC(_img_meta, cpu_only=True))
                    # proposals.append(_proposal)
                    if self.flip_ratio > 0:
                        _img, _img_meta = prepare_rotation_single(
                            img, scale, True, proposal, angle,img_info)
                        imgs.append(_img)
                        img_metas.append(DC(_img_meta, cpu_only=True))
