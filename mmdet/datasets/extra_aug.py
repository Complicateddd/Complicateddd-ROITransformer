...
class RandomCrop2(object):
    ...
    def __call__(self, img, boxes,mask_,labels):
        h, w, c = img.shape

        # polys = mask2poly(mask_)
        # polys_np = np.array(polys)

        while True:
            mode = random.choice(self.sample_mode)
            if mode == 1:
                return img, boxes, mask_,labels
            ...
            for i in range(50):
                ...
                #     if mask[i]:
                #         mask_m.append(mask_[i].astype(np.uint8))
                # print(mask.shape)
                # adjust boxes
                img = img[patch[1]:patch[3], patch[0]:patch[2]]
                ...
                for i in range(len(mask)):
                    ...
                    if mask[i]:
                        ...
                        mask_m.append(mask_[i][patch[1]:patch[3], patch[0]:patch[2]])

                # polys_np=polys_np[patch[1]:patch[3], patch[0]:patch[2]]


                # mask_m = poly2mask(polys_np[:, np.newaxis, :].tolist(), new_h, new_w)

                boxes[:, 2:] = boxes[:, 2:].clip(max=patch[2:])
                boxes[:, :2] = boxes[:, :2].clip(min=patch[:2])
                boxes -= np.tile(patch[:2], 2)

                return img, boxes,mask_m, labels

class PhotoMetricDistortion2(object):
    ...
    def __call__(self, img, boxes, mask,labels):
        # random brightness
        if random.randint(2):
            delta = random.uniform(-self.brightness_delta,
                                   self.brightness_delta)
    ...
    def __init__(self,
        ...
        if photo_metric_distortion is not None:
            self.transforms.append(
                PhotoMetricDistortion2(**photo_metric_distortion))
        if expand is not None:
            self.transforms.append(Expand(**expand))
        if random_crop is not None:
            self.transforms.append(RandomCrop2(**random_crop))

    def __call__(self, img, boxes,mask,labels):
        img = img.astype(np.float32)
        for transform in self.transforms:
            img, boxes,mask,labels = transform(img, boxes,mask,labels)
        return img, boxes,mask,labels
